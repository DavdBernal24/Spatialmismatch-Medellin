...
### Note: Even if the cost says 10, it is looping trough different distances
### Purpose : This code creates a function for public transport TA



compute_ta <- function(sits_complete, travel_times, travel_distances, costs, emp,
                       cost_over_10, cost_under_10, adjustment,ipc,
  ...
  ## Assigning costs
  times_distances$transpcost = ifelse(times_distances$dist >= distance, cost_over_10*adjustment, cost_under_10*adjustment)
  ...
  write.csv(times_distances, file = paste0(ruta, "TA_components_public_dist",distance, "_", year, ".csv"), row.names = FALSE)
  
  write.csv(ta_info, file = paste0(ruta,filename), row.names = FALSE)
  ...
  write.csv(ta_time, file = paste0(ruta, "TA_Public_OnlyTime_", year, ".csv"), row.names = FALSE)
