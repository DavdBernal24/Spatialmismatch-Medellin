...
# Script name:        35_Dependent_Boostrap_TA_Public_v4.R
...
ta_pub_12 <- numeric(bs)
ta_pub_pc_12 <- numeric(bs)
ta_pub_17 <- numeric(bs)
ta_pub_pc_17 <- numeric(bs)
...
for (iter in 1:bs) {
  ...
  ## ----- Travel times
  pub_times = read_csv("T:/Spatial Mismatch/Base/travel_times_public_2012.csv")
  ...
  varname <- "tapub2012"
  percap_varname <- "tapub2012_percap"
  ...
  #e2bs = rnorm(n=1, mean=e2, sd = se)
  #e2bs_17 = rnorm(n=1, mean=e2_17, sd = se)
  ...
  ta_pub_2012 <- ta_info
  ta_pub_2012 <- ta_pub_2012 %>% dplyr::select(-c(empbs))
  ...
  # nos quedamos con el empleo bootstrapeado de 2017
  #emp_2_17 <- emp_2 %>% dplyr::select(k, emp_2_17)
  ...
  year = 2017
  varname <- "tapub2017"
  percap_varname <- "tapub2017_percap"
  ...
  ## ----- Travel times
  priv_times = read_csv("T:/Spatial Mismatch/Base/bing_travel_duration_full.csv")
  pub_times = read_csv("T:/Spatial Mismatch/Data/times_2017_w2012_v2.csv")
  ...
  d = read_csv("T:/Spatial Mismatch/Base/mean_distance_border.csv", col_types = cols(X1 = col_skip()))
  ...
  #----------------------------------------------------------------
  ta_pub_12[iter] <- mean(ta_pub_2012$tapub2012)
  ta_pub_pc_12[iter] <- mean(ta_pub_2012$tapub2012_percap)
  
  ta_pub_17[iter] <- mean(ta_pub_2017$tapub2017)
  ta_pub_pc_17[iter] <- mean(ta_pub_2017$tapub2017_percap)
...
results <- data.frame(ta_pub_12, ta_pub_pc_12, ta_pub_17, ta_pub_pc_17)
...
write.csv(results, 
          "//bmdgiesan/DGIESAN/PROYECTOS/DASPERI/DatosInvestigación/Spatial Mismatch/Base/bootstrap_dependiente_TA_Public_remuestreo_parejas_2025.csv")
...
results_final <- fread("//bmdgiesan/DGIESAN/PROYECTOS/DASPERI/DatosInvestigación/Spatial Mismatch/Base/bootstrap_dependiente_TA_Public_remuestreo_parejas_2025.csv")
...
results_final <- results_final %>% mutate(diff_public_adj = mean_adj_2017 - mean_adj_2012)
results_final <- results_final %>% mutate(pct_diff_public_adj = 100*(mean_adj_2017 - mean_adj_2012)/mean_adj_2012)
...
results_final <- results_final %>% mutate(lower_ci_mean_adj_2012 = quantile(mean_adj_2012, 0.025),
                                          ...
                                          lower_ci_pct_change = quantile(pct_diff_public_adj, 0.025),
                                          upper_ci_pct_change = quantile(pct_diff_public_adj, 0.975))

# generamos medias para tenerlas ahi
results_final <- results_final %>% mutate(mean_public_2012 = mean(mean_2012), mean_public_2017 = mean(mean_2017),
                                          mean_public_2012_adj = mean(mean_adj_2012), mean_public_2017_adj = mean(mean_adj_2017),
                                          mean_diff = mean(diff_public_adj), mean_diff_pct = mean(pct_diff_public_adj))


# variables de interes
results_final <- results_final %>% dplyr::select(mean_public_2012, mean_public_2017, mean_public_2012_adj, mean_public_2017_adj, mean_diff, mean_diff_pct,
...
# duplicates drop
results_final <- results_final %>% distinct(mean_public_2012, .keep_all = TRUE)


# guardamos csv para tex
write.csv(results_final, 
          "//bmdgiesan/DGIESAN/PROYECTOS/DASPERI/DatosInvestigación/Spatial Mismatch/Base/bootstrap_dependiente_TA_Public_for_tab_remuestreo_parejas_2025.csv")
